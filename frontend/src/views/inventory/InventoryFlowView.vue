...
const inboundRows = computed(() => rows.value.filter((item) => ['PURCHASE_INBOUND', 'INITIAL', 'CHECK'].includes(item.bizType)))
const outboundRows = computed(() => rows.value.filter((item) => ['SALES_OUTBOUND', 'OUTBOUND'].includes(item.bizType)))
const transferRows = computed(() => rows.value.filter((item) => item.bizType === 'TRANSFER'))
