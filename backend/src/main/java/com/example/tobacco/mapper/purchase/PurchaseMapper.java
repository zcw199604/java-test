...
public interface PurchaseMapper {
    ...
    @Update("update purchase_orders set supplier_id=#{supplierId}, product_id=#{productId}, quantity=#{quantity}, unit_price=#{unitPrice}, total_amount=#{totalAmount}, status='CREATED', audited_by=null, audited_at=null, audit_remark=null, cancel_reason=null where id=#{id}")
    ...
    Integer selectInventoryQuantity(@Param("productId") Long productId);

    @Select("select count(1) from inventories where product_id=#{productId}")
    Integer countInventories(@Param("productId") Long productId);

    @Update("update inventories set quantity=#{quantity}, warning_threshold=(select warning_threshold from products where id=#{productId}), warehouse_name='中心仓' where product_id=#{productId}")
    void updateInventoryForInbound(@Param("productId") Long productId, @Param("quantity") Integer quantity);

    @Insert("insert into inventories(product_id,warehouse_name,quantity,warning_threshold) values(#{productId},#{warehouseName},#{quantity},(select warning_threshold from products where id=#{productId}))")
    void insertInventoryForInbound(@Param("productId") Long productId, @Param("warehouseName") String warehouseName, @Param("quantity") Integer quantity);

    @Update("update purchase_orders set status='INBOUND', inbound_at=now() where id=#{id}")
    void inboundOrder(@Param("id") Long id);
