...
### 新增
- 新增销售出库独立选仓页面，支持从销售订单列表进入并选择仓库执行出库
- 新增统一追溯查询增强：`/api/reports/compliance-trace` 现支持关键字、仓库、业务类型与节点类型筛选
- 新增采购订单与销售订单详情查看页，拥有查看权限的角色可直接查看订单明细

### 变更
- 库存调拨与库存盘点现已同步写入统一追溯节点，库存台账与合规追溯形成“库存变化 + 业务节点”双视角
- `GET /api/inventory-records` 新增 `keyword` 筛选，库存追溯台账支持按商品、仓库、备注和操作人过滤

### 修复
- 修复销售订单页点击“出库”后缺少前端路由承接的问题，销售出库闭环已可直接从前端完成
- 修复销售订单列表将 `status` 误标记为“回款状态”的文案问题，现统一更正为“订单状态”
- 修复采购/销售订单详情页未展示仓库信息与流程时间的问题，现已补充详情字段与时间线
...
- 整理 README 与当前知识库，按真实代码口径修正文档中的认证方式、数据访问方式、导出格式、接口清单与数据模型描述
