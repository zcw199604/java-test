...
public class InventoryService {
    ...
    @Transactional
    public String transfer(InventoryChangeRequest request) {
        ...
        jdbcTemplate.update("insert into inventory_records(product_id,biz_type,biz_id,change_qty,before_qty,after_qty,remark) values(?,?,?,?,?,?,?)", request.getProductId(), "TRANSFER", null, 0, beforeQty, afterQty, request.getRemark());
        return "调拨记录已登记";
    }

    @Transactional
    public String check(InventoryChangeRequest request) {
        Integer beforeQty = jdbcTemplate.queryForObject("select quantity from inventories where product_id=?", Integer.class, request.getProductId());
        if (beforeQty == null) throw new IllegalArgumentException("库存不存在");
        ...
        jdbcTemplate.update("insert into inventory_records(product_id,biz_type,biz_id,change_qty,before_qty,after_qty,remark) values(?,?,?,?,?,?,?)", request.getProductId(), "CHECK", null, afterQty - beforeQty, beforeQty, afterQty, request.getRemark());
        return "盘点结果已更新";
    }
}
