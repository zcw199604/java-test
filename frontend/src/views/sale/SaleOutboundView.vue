  ...
  <div class="page-stack">
    <PageSection title="销售出库" description="仅管理员、超级管理员或库管可对已审核通过的销售单执行出库确认并自动扣减库存。">
      ...
      <div class="dialog-footer">
        <el-button v-permission="['sale:edit', 'inventory:edit']" type="primary" :disabled="!selectedId || !warehouseId" @click="handleOutbound">确认出库</el-button>
      </div>
    </PageSection>
  </div>
</template>
...
const loadData = async () => {
  ...
  warehouseId.value = outboundRows.value[0]?.warehouseId || warehouseOptions.value[0]?.id || null
}

const handleOutbound = async () => {
  await outboundSales(selectedId.value, { warehouseId: warehouseId.value, deliveryInfo: deliveryInfo.value })
