...
public class SalesService {
    ...
    @Transactional
    public SalesOrderItem outbound(Long id) {
        ...
        jdbcTemplate.update("insert into inventory_records(product_id,biz_type,biz_id,change_qty,before_qty,after_qty,remark) values(?,?,?,?,?,?,?)", item.getProductId(), "SALES_OUTBOUND", id, -item.getQuantity(), beforeQty, afterQty, "销售出库");
        return detail(id);
    }

    @Transactional
    public SalesOrderItem payment(Long id, PaymentRequest request) {
        SalesOrderItem item = detail(id);
